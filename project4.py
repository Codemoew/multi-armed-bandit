...
def UCB_new(T,k,df):
    ...
    for i in range(k):
        ...
        mu[i]=df.loc[:,i][i]
        exp_reward[i]=df.loc[:,i][i]
        exp_reward_squre[i]=exp_reward[i]*exp_reward[i]
        temp+=best_reward-exp_reward[i]
        regret[i]=temp/(i+1)
    for t in range(1,T-k+1):
        V=np.sqrt(2*np.log(t+k)/n)+exp_reward_squre/n-np.square(exp_reward/n)
        UCB=mu+np.sqrt(np.log(t+k)/n*V)
        j=np.argmax(UCB)
        if V[j]>0.25:
            UCB=mu+np.sqrt(np.log(t+k)/n*0.25)  
            j=np.argmax(UCB)
        I_t[k-1+t]=j
        n[j]+=1
        exp_reward[j]+=df.loc[:,t][j]
        exp_reward_squre=np.square(exp_reward)
        mu[j]+=1/n[j]*(df.loc[:,t][j]-mu[j])
        temp+=best_reward-exp_reward[j]/n[j]
        regret[k-1+t]=temp/(k+t)
    print("UCB_new total reward",exp_reward.sum())
    return I_t,regret
        
I_t,regret=UCB_new(T,k,df)
plt.plot(regret,label='UCB_new')
plt.legend(loc='best')
plt.title('regret/t vs number of rounds')
plt.xlabel('number of rounds')
plt.ylabel('regret/t')    